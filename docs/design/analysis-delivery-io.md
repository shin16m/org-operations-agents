...
| profile | 開発が consume するもの | テンプレ |
|---------|------------------------|----------|
| `model-serve` / `full` | カタログ · モデル · 推論 API | [`cross-team-artifact-bridge.md`](cross-team-artifact-bridge.md#分析--開発) |
| **`insights`** | **DashboardBundle JSON** + 生データパス | [`cross-team-artifact-bridge.md`](cross-team-artifact-bridge.md#insights--full-ui-ダッシュボードprofile-insights-連携) · [`insights-dashboard-consume-io.md`](insights-dashboard-consume-io.md) |

`insights` 完了時は `output/analysis/bundles/<task_gid>-dashboard-bundle.json` を生成し、`DeptWorkComplete.artifacts[]` に含める。
...
**下流 full-ui 連携時:** 探索完了後に [`DashboardBundle`](../../schemas/analysis/dashboard-bundle.v1.schema.json) を `output/analysis/bundles/` へ出力（[`insights-dashboard-consume-io.md`](insights-dashboard-consume-io.md)）。
...
| DashboardBundle（full-ui 連携時） | data-analyst / analytics-pm | `output/analysis/bundles/<task_gid>-dashboard-bundle.json` |
