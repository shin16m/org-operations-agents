...
**背景:** `approval-flow E2E ドライラン` で同名の別 field（標準 assignee 関連等）に誤マッチして project-private GID を返す事象が発生したため、name 一致だけでは不十分という前提で書き換えた（[`docs/verification/approval-flow-e2e-dryrun.md`](../verification/approval-flow-e2e-dryrun.md) §6 を参照）。
...
> 同 workspace 内では Agent Type CF が共有されるため `1214771428861230` と同 GID。`sync_assignee_type_env.py` v1.7 で正しく検出されることを確認済み（[delivery](../verification/sync-assignee-type-fix-delivery.md)）。
