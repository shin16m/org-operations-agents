...
**プロジェクトごとに CF GID が異なる。** 切替時は [`tools/sync_assignee_type_env.py`](../../tools/sync_assignee_type_env.py) で `.env` を同期する（lookup は `Agent Type` · 旧称 `担当種別` フォールバック）。
...
| 種別 | GID |
|------|-----|
| フィールド「Agent Type」（旧 担当種別） | `1215102364986855` |
