...
| `intake_mode` | enum? | `natural_language`（本番既定）\| `task_creation_request` \| `epic_input` \| **`document_request`（文書化 · Epic 省略）** \| `asana_task`（URL 手動渡し · 遂行・起票）。詳細: [`wakuoke-intake-modes.md`](wakuoke-intake-modes.md)。**チャット入口**では planning gate デフォルト省略 · `--record-wait` / SuspendedSession は**使わない**（[`chat-driven-ops.md`](chat-driven-ops.md)） |
