...
**依頼者が最初に読むのは人間向けブロック。** 機械向け署名は**末尾**に置く。
...
phase: complete
executed_at: 2026-05-24T09:00:00+09:00
---
```
...
|----------|------|------|
| `## 依頼者向け` + 本文 | **先頭** | 依頼者（人間） |
| `agent-work-record` 署名 | **末尾** | エージェント / 監査 / CLI |

**表示名 SSOT:** [`workflows/agent-display-names.yaml`](../../workflows/agent-display-names.yaml) — registry `slug` → 日本語表示名（`comment_task.py` / `format_signed_comment` が参照）。

### 3.1 署名ブロック（必須 · 末尾）

コメント末尾に YAML 風メタデータを置く（Asana 上はプレーンテキスト）。
...
**CLI 強制（2026-06-06）:** `comment_task.py` が notes `担当:` と `--agent` / `--skill` を検証（不一致は exit 4）。`complete_task.py` はワーカー subs で署名 comment + md attach を検証。詳細: [`pm-worker-separation-enforcement.md`](pm-worker-separation-enforcement.md)。
