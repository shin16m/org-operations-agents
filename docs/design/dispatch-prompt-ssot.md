...
3. **check_pm_review_gate.py** exit 0（デフォルト gate 無し）
   **opt-in**（plan `"human_review_gate": true` / `--require-human-review` / `ORG_OPS_PM_REVIEW_GATE=1`）:
   **create_pm_review_gate.py** --plan <上記 plan> [-y] → 人間 complete → check exit 0
4. 親 notes → 担当: ux-pm · 状態: in_progress
...
2. delivery profile を決定（full / full-ui / lite / doc-only）。full-ui は ## 依存（UX）未転記なら着手しない
   full-ui 時: `python tools/pm_intake_gate.py --gid {task_gid} --plan <plan.json>` が exit 0 であること（`pm_assign_subtasks` 内でも自動チェック）
...
4. **check_pm_review_gate.py** exit 0（デフォルト gate 無し）
   **opt-in**（plan `"human_review_gate": true` / `--require-human-review` / `ORG_OPS_PM_REVIEW_GATE=1`）:
   **create_pm_review_gate.py** --plan <上記 plan> [-y] → 人間 complete → check exit 0
...
【L3b — ワーカー dispatch（必須）】**check_pm_review_gate exit 0 後**。サブタスク作成直後に PM セッションで worker 作業を続けない。
...
3. **check_pm_review_gate.py** exit 0（デフォルト gate 無し）
   **opt-in**: **create_pm_review_gate.py** --plan <上記 plan> [-y] → 人間 complete → check exit 0
4. 親 notes → 担当: analytics-pm · 状態: in_progress
...
4. **check_pm_review_gate.py** exit 0（デフォルト gate 無し）
   **opt-in**: **create_pm_review_gate.py** --plan <上記 plan> [-y] → 人間 complete → check exit 0
5. 親 notes → 担当: audit-pm · 状態: in_progress
...
2. サブ notes の done_when に従い成果物を作成（PM 代行禁止）
3. requirements-writer: attach_task_files.py（comment 前 · worker + review サブ）— [`pm-worker-separation-enforcement.md`](pm-worker-separation-enforcement.md)
4. comment_task.py --agent {worker_slug} --skill {skill_path_from_registry} → PM へ報告
   ※ CLI が notes 担当: と --agent 不一致を拒否（exit 4）
