...
[`workflows/development-delivery.yaml`](../../workflows/development-delivery.yaml) v3 に従い（[`development-delivery-io.md`](../design/development-delivery-io.md)）:

PM（profile 決定）→ 要件 → … → **ux-reviewer（full-ui）** → 検証 → 事後仕様 → `DeptWorkComplete`
