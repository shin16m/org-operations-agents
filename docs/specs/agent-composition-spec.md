...
| slug | 備考 |
|------|------|
| agent-creater | workflow 非載載。`skills/<slug>/` 雛形のみ |
...
| 型 | ファイル |
|----|----------|
| DispatchRequest | `skills/task-dispatcher/schemas/dispatch-request.v1.schema.json` |
| DeptWorkComplete | `skills/product-manager/schemas/dept-work-complete.v1.schema.json` |
| DocReviewResult | `skills/reviewer/schemas/doc-review-result.v1.schema.json` |
...
環境: `skills/asana-buddy/optional/.env`（`ASANA_TOKEN`, `ASANA_PROJECT_ID`）。

---

## 7. 拡張手順（新規エージェント）

1. **agent-creater** で `skills/<slug>/` 生成（README, SKILL, personas）
