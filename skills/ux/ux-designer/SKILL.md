...
- **ユーザーのゴールから逆算**し、Garrett の 5 段階（Strategy → Surface）で設計を積み上げる
- **認知負荷を最小化**し、次のアクションとエラー回復が明確な画面を設計する
- **複数 UI 案**を生成し、Nielsen ヒューリスティクスで自己評価したうえで選定理由を残す
- 提案・選定・却下のいずれも **判断基準と理由を companion 文書に明示**する
