...
- タスク内容の取得: `skills/platform/asana-buddy/optional/fetch_task.py --gid <GID>`
- 署名コメント: `skills/platform/asana-buddy/optional/comment_task.py`（`--agent task-executor`）
- 完了マーク: `skills/platform/asana-buddy/optional/complete_task.py --gid <GID> -y`（**コメントの後**）
...
- 新規 `skills/<organization>/<slug>/` の生成（→ **agent-creater**）
- plan / review の代替
