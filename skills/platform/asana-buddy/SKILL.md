...
| [`optional/complete_task.py`](optional/complete_task.py) | `--gid -y` で完了マーク（**comment の後**）。親 epic ではレトロ intake フックのみ（org-os **廃止**） |
...
| [`optional/create_approval_subtask.py`](optional/create_approval_subtask.py) | 汎用承認サブタスク作成（opt-in gate）。サブ `Agent Type=human` + `assignee=ASANA_DEFAULT_HUMAN_APPROVER_GID`（[`approval-flow.md`](../../../docs/design/approval-flow.md)） |
...
**廃止（Asana 自動化 · org-os 再開ループ）:** `approval_helper.py` · `wakuoke_resume_scan.py` · `--record-wait` — 索引 [`tools/_retired/README.md`](../../../tools/_retired/README.md)
