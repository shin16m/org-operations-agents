...
| [`optional/complete_task.py`](optional/complete_task.py) | `--gid -y` で完了マーク（**comment の後**）。親 epic では完了直前に `org-os complete --allow-skip` を自動実行 |
...
| [`optional/create_approval_subtask.py`](optional/create_approval_subtask.py) | 汎用承認サブタスク作成。**作成と同時に親 `OS State=Waiting` + `Approval Required=Yes`、サブ `Agent Type=human` + `assignee=ASANA_DEFAULT_HUMAN_APPROVER_GID` を一括設定**（[`approval-flow.md`](../../../docs/design/approval-flow.md)） |
| [`optional/check_approval_subtask.py`](optional/check_approval_subtask.py) | 汎用承認サブ polling（1 回チェック） |
| [`tools/approval_helper.py`](../../../tools/approval_helper.py) | 承認サブ完了監視 + 親 CF 戻し + ログ JSON。`--once` / `--interval` / `--timeout` オプションでオンデマンド運用（[`approval-flow.md` §5.1](../../../docs/design/approval-flow.md)） |
| [`tools/wakuoke_resume_scan.py`](../../../tools/wakuoke_resume_scan.py) | Ready epic 再開スキャナ。ヘルパーログと突合 → RESUME / ESCALATE 表示 + NG counter 永続化（[`approval-flow.md` §5.3](../../../docs/design/approval-flow.md)） |
...
**コメント本文:** `--summary` だけにせず、`--body`（または `--body-file` / `--action` 繰り返し）に **§4 依頼者向け本文**（実施内容 · 成果物 · 次の状態。レビュアー/PM は判断・理由）を書く。**です・ます調の自然な日本語**（§4.4–4.5 · NG/OK）。詳細は [`docs/design/agent-asana-comment-signature.md`](../../../docs/design/agent-asana-comment-signature.md) v1.3 · [`build_human_comment_body`](optional/asana_program_common.py) · [`docs/design/dept-work-io.md`](../../../docs/design/dept-work-io.md)。
