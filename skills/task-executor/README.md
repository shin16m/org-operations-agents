...
2. タスク内容を取得:
   ...
   ```powershell
   .\.venv\Scripts\python.exe .\skills\asana-buddy\optional\fetch_task.py --gid <TASK_GID>
   ```
...
4. 完了マーク（**エージェントが `done_when` 達成と判断したら `-y` で実行してよい**）:
   ...
   ```powershell
   .\.venv\Scripts\python.exe .\skills\asana-buddy\optional\complete_task.py --gid <TASK_GID> -y
   ```
