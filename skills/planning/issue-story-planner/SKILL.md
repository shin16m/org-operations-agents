...
| `subtasks[].department` | いいえ | **v1.2 のみ** — `planning` \| `ux` \| `development` \| `analysis` \| `audit`。task-dispatcher のルーティング用。Web Epic では **UI 系 development 子より `ux` 子を先**に並べる。組織変更 Epic では **`audit` 子を execution 系の最後**に置く。未設定時は [`organizations.yaml`](../../../workflows/organizations.yaml) の `pillar_defaults` で推定。 |
