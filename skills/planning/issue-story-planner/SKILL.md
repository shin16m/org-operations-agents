...
- **Execution Order CF**: 投入時に **数値 CF（1, 2, 3…）** と notes `着手順:` を自動設定。**タスク名に `【N/M】` は載せない**（[`org-os-product-io.md`](../../../docs/design/org-os-product-io.md) §5）。
...
- **子タスクの Asana `notes`**: スキーマ上は `background` / `summary` / `done_when` に分割して保持する。**消費側**（[`assemble_subtask_notes`](../../platform/asana-buddy/optional/asana_program_common.py)）は Asana `notes` 用に **二層 Markdown** で連結する（[`agent-asana-comment-signature.md`](../../../docs/design/agent-asana-comment-signature.md) §6.1）:

```markdown
## 依頼者向け（人間が最初に読む）

{summary を平易に — 依頼者が最初に読む}
...
```
  ...
  `pillar` / `department` がある場合は先頭に `柱:` / `チーム:` 行を付けてから上記ブロックを続ける。`epic.notes_markdown` も先頭 `## 依頼者向け` 必須（詳細は §6.1）。
...
| `subtasks[].department` | いいえ | **v1.2 のみ** — `planning` \| `ux` \| `development` \| `analysis` \| `governance` \| `audit`。task-dispatcher のルーティング用。Web Epic では **UI 系 development 子より `ux` 子を先**に並べる。組織変更 Epic では **`governance` 子（org-meta）を audit の直前**に置く。未設定時は [`organizations.yaml`](../../../workflows/organizations.yaml) の `pillar_defaults` で推定。 |
...
このスキルは **設計・文書化** に特化する。Asana トークンやプロジェクト GID は [`asana-buddy`](../../platform/asana-buddy/SKILL.md) 側の運用に従う。
...
**本スキルは新規 `skills/<organization>/<slug>/` を生成しない** → [`agent-creater`](../agent-creater/SKILL.md)。
