#!/usr/bin/env python3
"""Check planning gate (handoff_approved) subtask completion.
...
from planning_approval_gate_util import find_planning_approval_gate_sub  # noqa: E402


def main() -> int:
    p = argparse.ArgumentParser(description="Check planning approval gate")
    ...
    token = get_token()
    sub = find_planning_approval_gate_sub(args.parent, token)
    ...
    if sub is None:
        ...
        if args.json:
            ...
            print(
                console_safe(
                    "SKIP  planning_approval_gate  reason=no_gate_sub  (opt-out default OK)"
                )
            )
        return 0
